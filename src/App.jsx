...
`;
        ...
        {pmView==="generator"&&<div style={{flex:1,overflowY:"auto",padding:"32px 40px"}}>
          <div style={{maxWidth:600,margin:"0 auto"}}>
            ...
            {pgQuantumSafe&&pgStrength&&<div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:2,marginBottom:16,borderRadius:14,overflow:"hidden",border:"1.5px solid rgba(16,185,129,0.25)",boxShadow:"0 4px 20px rgba(0,0,0,0.2),0 0 20px rgba(16,185,129,0.08)"}}>
              {[{label:"Length",value:pgResult.length+" chars",icon:"📏"},{label:"Entropy",value:pgStrength.bits+" bits",icon:"🎲"},{label:"Classical",value:pgStrength.time,icon:"🖥️",color:pgStrength.color},{label:"Quantum",value:pgStrength.qTime,icon:"⚛️",color:pgStrength.qColor}].map((s,i)=><div key={i} style={{padding:"14px 12px",background:"rgba(16,185,129,0.04)",borderLeft:i>0?"1px solid rgba(16,185,129,0.12)":"none"}}><div style={{fontSize:10,color:T.dim,fontWeight:600,letterSpacing:0.5,marginBottom:6}}>{s.icon} {s.label}</div><div style={{fontSize:13,fontWeight:700,color:s.color||T.text,lineHeight:1.3,wordBreak:"break-word",textShadow:s.color?`0 0 8px ${s.color}40`:"none"}}>{s.value}</div></div>)}
            </div>}
  ...
  if(infoPage&&(authMode!=="app"||infoPage==="password-manager")){
    ...
    const infoPages={
      ...
      "password-manager":<>
        <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:0,marginTop:-16}}>
          ...
          <div>
            <h1 style={{...infoH,background:`linear-gradient(135deg,${T.dark?T.text:"#e2e8f0"} 30%,${pgQuantumSafe?"#10b981":T.accent} 70%,${pgQuantumSafe?"#059669":T.accent2||T.accent})`,WebkitBackgroundClip:"text",backgroundClip:"text",WebkitTextFillColor:"transparent",color:"transparent",textShadow:"none",margin:0,fontSize:32}}>ShieldCraft</h1>
            <p style={{fontSize:9,color:pgQuantumSafe?"rgba(16,185,129,0.6)":T.dim,margin:"2px 0 0",letterSpacing:1.2,fontWeight:500}}>Password Generator Tool by NotesCraft</p>
          </div>
        </div>
        <p style={{...infoP,color:"rgba(176,190,201,0.8)",marginTop:4}}>Generate strong passwords and securely store your credentials with end-to-end encryption.</p>
