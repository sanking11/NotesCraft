        ...
        <div style={{position:"relative",zIndex:1,maxWidth:infoPage==="security-blog"?"100%":800,margin:"0 auto",padding:infoPage==="security-blog"?"80px 0 60px":infoPage==="password-manager"?"28px 24px 60px":"100px 24px 60px"}}>
          ...
          {infoPage==="password-manager"&&(()=>{
            const feats=[
              ...
              {icon:"📱",title:"Scan-to-Transfer QR",color:"#a855f7",rgb:"168,85,247",desc:"Send the password to your phone via a local QR. Generated on-device, never hits any server.",detail:"The QR is encoded by JavaScript running in your tab — no api.qrserver.com, no third-party endpoint. Your password never leaves the device. When you close the modal the QR is destroyed in memory. The only ‘things’ that ever see the password: your screen pixels and your phone's camera looking at them."},
              {icon:"🧠",title:"3 Generation Modes",color:"#14b8a6",rgb:"20,184,166",desc:"Random, Memorable (word-based), or PIN. Pick the right style for the account — plus custom word lists.",detail:"Random: each character independently chosen from your charset — maximum entropy per character. Memorable: word-based passphrases like correct-horse-battery-staple — fewer bits per character but vastly easier to remember and type, and longer length compensates. PIN: pure numeric for ATMs and device-unlock codes. Each mode lets you trade entropy density against usability without ever sacrificing cryptographic randomness."},
