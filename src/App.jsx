...
`;
  ...
  if(infoPage==="cipher-craft"){
    ...
    const whyFeats=[
      ...
      {icon:"⚛️",t:"Quantum-Ready Mode",d:"AES-256 keeps ~128-bit strength even against Grover's algorithm. Quantum mode also raises Argon2id to 256 MiB / 4 passes, widening the margin against future hardware."},
    ...
    const faqs=[
      ...
      ["Why Argon2id and AES-256-GCM?","Argon2id is the winner of the Password Hashing Competition and the algorithm OWASP recommends for turning a passphrase into a key. Unlike older functions, it's memory-hard — it forces an attacker to spend large amounts of RAM per guess, which neutralises the cheap massively-parallel GPU/ASIC cracking rigs that make PBKDF2 guessing fast. The file itself is then sealed with AES-256-GCM, a NIST-standard authenticated cipher that's hardware-accelerated on virtually every modern CPU and detects any tampering. Best-in-class KDF + best-in-class cipher."],
      ...
      <div style={{width:"100%",height:"100vh",overflowY:"auto",overflowX:"hidden",background:T.dark?T.bg:"#0a0a12",color:txt,fontFamily:`${F.body},sans-serif`,position:"relative"}}>
        ...
        <div style={{position:"relative",zIndex:1,maxWidth:920,margin:"0 auto",padding:"52px 24px 70px"}}>
          ...
          <div style={{display:"flex",flexWrap:"wrap",justifyContent:"center",gap:10,margin:"28px 0 8px"}}>
            {["✓ Nothing uploaded","✓ No account","✓ Works offline","✓ Argon2id","✓ AES-256-GCM","✓ Quantum-ready"].map((p,i)=><span key={i} className="lq-chip cc-pill" style={{color:sub,fontSize:11}}>{p}</span>)}
          </div>
  ...
  if(infoPage&&(authMode!=="app"||infoPage==="password-manager")){
    ...
    const infoPages={
      ...
      terms:<>
        ...
        <p style={infoP}>NotesCraft is an encrypted productivity suite that provides: (a) end-to-end encrypted note-taking with rich text editing and markdown support; (b) encrypted calendar management with event scheduling and reminders; (c) ShieldCraft — a full-featured password manager with credential storage, password generation (Random, Memorable, and PIN modes), TOTP 2FA authenticator, breach monitoring, vault organization, Ghost Shield hidden vaults, and import/export capabilities; (d) CipherCraft — a standalone, fully in-browser file encryption tool that seals any file with AES-256-GCM and a passphrase-derived key, with an optional quantum-resistant mode and a built-in passphrase generator; and (e) cross-device synchronization for the notes, calendar, and ShieldCraft services.</p>
